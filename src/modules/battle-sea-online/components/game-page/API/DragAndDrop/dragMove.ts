...
const findShip = (id: number, settedShips?: IShip[]) => {
  if (settedShips) {
    const ship = settedShips.find((ship) => ship.shipLocation.includes(id));
    console.log("ship", ship);
    return { shipLength: ship?.decks, shipLocation: ship?.shipLocation };
  }
  return {};
};
...
const getTouchOffset = (e) => {
  ...
  const event = e.changedTouches.length ? e.changedTouches : e.touches;
  const offsetX = event[0]?.clientX - rect.left;
  const offsetY = event[0]?.clientY - rect.top;

  return { offsetX, offsetY };
};

export const dragOver: IDragHandler = (event, settedShips) => {
  ...
  const { shipLength, shipLocation } = findShip(
